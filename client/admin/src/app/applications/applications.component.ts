...
import { ApplicationsService, AlertService } from '@app/_services';
...
export class AppliedComponent {
    name: string;
    editObj:any={};
    constructor(private alertService: AlertService,private applicationsService: ApplicationsService, public dialog: MatDialog) {
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(appliedDialog, {
            height: '80%',
            width: '50%',
        });
    ...
    saveData(data){
        this.applicationsService.save(data)
            .pipe(first())
            .subscribe(
                data => {
                    this.getData();
    ...
    getData(){
        this.applicationsService.getAll().subscribe(res=>{
            let result:any =res;
    ...
    editData(id){
        this.applicationsService.getById(id).subscribe(res=>{
            const dialogRef = this.dialog.open(appliedDialog, {
                data:res.data,
                height: '80%',
                width: '50%',
    ...
    updateData(data){
        this.applicationsService.update(data._id,data)
        .pipe(first())
        .subscribe(
            data => {
                this.getData();
...
@Component({
    selector: 'job-dialog',
    templateUrl: 'applications.model.html',
    styleUrls: ['applications.component.css']
})

export class appliedDialog {
    public FormGroup: FormGroup;

    constructor(
        public dialogRef: MatDialogRef<appliedDialog>,
